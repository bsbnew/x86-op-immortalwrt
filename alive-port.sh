...
deploy_service() {
    ...
    # 确保 supervisord.conf 包含 include 配置
    info "检查 supervisord.conf 配置..."
    fix_supervisord_conf
...
# ----------------------------------------------------------
# 修复 supervisord.conf 配置
# ----------------------------------------------------------
fix_supervisord_conf() {
    # 检查 supervisord.conf 是否存在
    if [ ! -f /etc/supervisord.conf ]; then
        warn "/etc/supervisord.conf 不存在，尝试创建默认配置..."
...
EOF
        ok "已创建默认 supervisord.conf"
    else
        # 检查是否包含 include 配置
        if ! grep -q "^\[include\]" /etc/supervisord.conf 2>/dev/null; then
            info "添加 include 配置到 supervisord.conf..."
            ...
            # 检查 files 配置
            if ! grep -q "files = .*supervisor.d" /etc/supervisord.conf 2>/dev/null; then
                info "更新 include files 配置..."
                sed -i 's|^files = .*|files = /etc/supervisor.d/*.conf|' /etc/supervisord.conf || true
                ok "已更新 include files 配置"
            fi
        fi
    fi

    # 对于 Debian/Ubuntu 系统，使用 /etc/supervisor/conf.d
...
EOF
            ok "已创建默认 supervisord.conf (Debian/Ubuntu)"
        else
            if ! grep -q "^\[include\]" /etc/supervisord.conf 2>/dev/null; then
                echo "" >> /etc/supervisord.conf
                echo "[include]" >> /etc/supervisord.conf
                echo "files = /etc/supervisor/conf.d/*.conf" >> /etc/supervisord.conf
                ok "已添加 include 配置 (Debian/Ubuntu)"
            fi
        fi
    fi
}
...
main_menu() {
    ...
    # 非交互模式下（如 curl | sh），尝试自动修复并显示状态
    if [ ! -t 0 ]; then
        # 检查服务是否正常运行
        if ! supervisorctl status "${SERVICE_NAME}" >/dev/null 2>&1; then
            info "检测到服务未正常运行，尝试自动修复..."
            fix_service
        fi
        show_status
        exit 0
